...
* Description: Lab 11 - Shared Memory (multiple processes)
******************************************************************************/
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
...
void sum_array(int *array, const int start_index, const int length) {
    //Create a local int variable called partial_sum to accumulate the sum.
    int partial_sum = 0;
    // One child process adds all values at even indexes & the other adds the values at odd indexes.
    // Proper use of start_index and length will enable you to write one loop.
    // *( similar to lab mtsumarry.c )*
    for (int i = start_index; i < length; i+=2) { // < or <= .... i= i + 2
        partial_sum = partial_sum + array[i];
    }
    // Use P and V from the semaphore library, and increment the global variable
    // after ensuring mutual exclusion.
    // similar to lines 86-88 in sleepingprocess.c
    P(semid);
    *sum = *sum + partial_sum;
    V(semid);
    // No error messages are required, as they come from the library.
...
int main(int argc, char *argv[]) {
    ...
    // *( similar to lines 105-110 sleepingprocesses.c )*
    key_t key;
    if ((key = ftok("./", 'a')) == -1) {
        // If it fails, print the error message:
        //   "Error: Cannot create shared memory key. '%s'.\n",
    ...
    // Create the semaphore semid. *( similar to sleepingprocesses.c lines 112-117 )*
    ...
    sum = (int *)shm;
    *sum = 0;  //??? like *num_running = 0; in sleepingproccesses?
    ...
    for (int i = 0; i < 2; i++) {
        ...
        // *( lines 138-151 mtsumarray )*
        if ((pid = fork()) < 0) {
            fprintf(stderr, "Error: process[%d] failed. %s.\n", i+1, strerror(errno));   // i+1 to make it 1 or 2 not 0 or 1
            continue; // like mpsumarray.c? or retval = exit_failure got exit
        } else if (pid == 0) {   
            // in child
            sum_array(array, i, num_values); // num_values or length?
            free(array);
            //num_started++;
            //continue;
        } else {  
    ...
    for (int i = 0; i < num_started; i++) {
        ...
        // from line 158 in sleepingprocesses.c
        //else {
...
EXIT:
    ...
    // *( lines 177 - 181 sleepingprocesses.c )*
